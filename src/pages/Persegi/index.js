 import { NavigationContainer } from '@react-navigation/native';
import React, { Component } from 'react';
 import {
   AppRegistry,
   StyleSheet,
   Button,
   Text, TextInput,
   View
 } from 'react-native';
 
 export default class Persegi extends Component {
   constructor(props){
     super(props)
     this.state = {
       sisi1:0,
       sisi2:0,
       luas:0
     };
   }
 
   render() {
     return (  
       <View style = {{flex:1,backgroundColor:'#bbdefb'}}>
 
         <View style={{backgroundColor:'#2196f3'}}>
            <Text style = {{padding: 10, fontSize: 20, color: 'white', textAlign:'center'}} >
             Menghitung luas Persegi
           </Text>
          </View>
         
         <View style={{margin:20,padding: 10, backgroundColor:'#e3f2fd'}}>
             <TextInput style = {{height: 40}}
               placeholder="Masukkan Sisi 1"
               onChangeText={(sisi1)=>this.setState({sisi1})}
               keyboardType = 'numeric'
             />
             <TextInput style = {{height: 40}}
               placeholder="Masukkan  Sisi 2"
               onChangeText={(sisi2)=>this.setState({sisi2})}
               keyboardType = 'numeric'
             />
            <View style={{margin:20}}>
             <Button
               onPress={()=>this.setState({
                 luas: (this.state.sisi1*this.state.sisi2)
               })}
               title="Hitung"
               accessibilityLabel="Klik untuk menghitung"
             />
            
            </View>
              <View style={{margin:20}}>
              <Button tittle="Kembali" onPress={ () => Navigation.replace('Home')}/>
              </View>
               
              
        </View>
 
         <View style={{margin:20, backgroundColor:'#90caf9'}}>
           <Text style = {{padding: 10, fontSize: 20}} >
               Sisi 1 =  {this.state.sisi1} {"\n"}
               Sisi 2 =  {this.state.sisi2} {"\n"}
               Luas = {this.state.luas}
           </Text>
          </View>
          
          
    </View> 
 
     );
   }
 
 }
 const styles = StyleSheet.create({})
 
 
 
 
 
 
 